...
- runs on a Raspberry Pi 3 Model B+ with Raspberry Pi OS / Linux
...
- pre-soldered 40-pin GPIO header

- 32 GB Class 10 microSD card for OS, code, logs, and test artifacts
...
- OUT is read through the ADS1115 ADC

External ADC:

- Adafruit ADS1115 16-bit, 4-channel ADC

- communicates over I2C

- default I2C address: 0x48

- power the ADS1115 from 3.3 V so its I2C pull-ups remain safe for Raspberry Pi GPIO

- initial optical connection: ALS-PT19 OUT -> ADS1115 A0

- do not substitute MCP3008 without explicitly redesigning the bus allocation

Sensor B — positioning:

- Adafruit VL6180X time-of-flight proximity/distance sensor
...
- use 3.3 V logic/power with the Pi prototype
...
- Adafruit PiTFT 2.2" 320x240 HAT Mini Kit, product #2315

- no touchscreen; user interaction is through physical tactile switches

- four on-board tactile switches are available

- use three for UP / DOWN / ENTER unless DESIGN.md intentionally changes the mapping

- final physical power-control implementation remains separate from a software shutdown button

PiTFT / bus allocation:

- PiTFT uses Raspberry Pi hardware SPI: SCK, MOSI, MISO, CE0, and CE1

- PiTFT also uses GPIO25

- therefore SPI is considered occupied by the display in the current design

- do not assume CE1 is available for an MCP3008 or other SPI peripheral

- ADS1115 and VL6180X instead share the I2C bus at distinct addresses:

    ADS1115: 0x48

    VL6180X: 0x29

GPIO access / prototyping:

- use a 2x20 stacking female header on the PiTFT instead of its plain included header

- the stacking header preserves GPIO access above the PiTFT

- current breadboard plan uses a 40-pin ribbon cable / GPIO breakout and solderless breadboard

- jumper-wire brand or pack is not a design dependency; standard 2.54 mm Dupont wiring is fine

- light sensor, ADC, VL6180X, and PiTFT/header assembly may require soldering for reliable breadboard use

Electrical safety:

- Raspberry Pi GPIO is 3.3 V only and is not 5 V tolerant

- verify rails with a multimeter before connecting new hardware

- wire or rewire GPIO/sensor connections only with power removed

Power:

- development and soak-test supply: regulated 5 V / 2.5 A micro-USB supply for Raspberry Pi 3 Model B+

- final handheld rechargeable-battery solution is NOT YET FROZEN

- final battery/power board must provide a stable regulated 5 V rail with adequate current headroom

- choose the final battery solution only after measuring real current draw of the assembled Pi + PiTFT + sensors

- device must support continuous external power for soak testing
...
- Treat the current hardware bus map as a design constraint: PiTFT owns the
hardware SPI bus/CE lines; ADS1115 and VL6180X share I2C.

- Do not move a sensor to a different bus or change voltage assumptions
without updating DESIGN.md and re-checking electrical compatibility.
...
    M2: add ADS1115 optical capture path
...
- final ALS-PT19 / ADS1115 sampling rate, gain, calibration, and optical-shroud geometry

- final rechargeable power board / battery capacity / final physical power switch
...
- final button-action mapping beyond the required UP / DOWN / ENTER controls
